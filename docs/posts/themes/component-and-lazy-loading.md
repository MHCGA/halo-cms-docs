...
src/components/pagination/
  ├── main.ts        ← 脚本 + 样式导入
  ├── main.css       ← 样式定义
  └── template.html  ← Thymeleaf 模板（两个片段）
```
...
<th:block th:insert="~{components/pagination/template :: body(...)}"></th:block>
```
...
- `th:fragment="html(title, head, content, header)"`：定义了一个根级片段，接收 4 个参数
- `th:insert="${head}"`：将页面传入的 head 片段内容插入
- `th:insert="${content}"`：将页面传入的 body 片段内容插入
- 这样每个页面都复用同一个 HTML 结构，避免重复代码
...
- Vite 扫描 HTML 中的 `<script type="module">` 标签
- 发现 `src="/src/scripts/pages/index.ts"` 后，将其作为一个独立的构建入口
- 分析 index.ts 中的所有 import（包括 CSS、其他脚本等）
- 为该页面生成独立的代码包
...
>
  ...
  <head th:remove="tag">
    ...
    <th:block th:insert="~{components/post-list/template :: head}"></th:block>
    <th:block th:insert="~{components/pagination/template :: head}"></th:block>
  </head>
  ...
  <body th:remove="tag">
    <div class="index-content">
      ...
      <!-- 该页面使用的组件的 body 片段 -->
      <th:block th:insert="~{components/post-list/template :: body(posts = ${posts})}"></th:block>
      <th:block th:insert="~{components/pagination/template :: body(currentPage = ${currentPage})}"></th:block>
    </div>
  </body>
</th:block>
```
...
4. Thymeleaf 渲染时，`th:replace` 会用 `layout.html` 的结构替换 `th:block` 标签
5. 传入的 `head` 和 `content` 参数会被插入到布局中对应的占位符处
...
2. **样式** (`main.css`)：组件的外观
3. **HTML 结构** (`template.html`)：组件的标签

这三个部分紧密相关，经常需要一起被引入。通过 Thymeleaf 的片段机制，我们可以在一个模板文件中定义两个片段，分别对应 "需要在 `<head>` 引入脚本和样式" 和 "需要在 `<body>` 显示 HTML"：

```html
<!-- templates/components/pagination/template.html -->
<!-- 
  片段 1：head 片段
  作用：在页面的 <head> 中引用这个片段时，会自动导入该组件的脚本
  Vite 会自动识别脚本中的 import 语句，并将关联的 CSS 也一起提取
-->
<head th:fragment="head">
  <script src="/src/components/pagination/main.ts" type="module"></script>
</head>
<!-- 
  片段 2：body 片段
  作用：在页面的 <body> 中引用这个片段时，会插入组件的 HTML 结构
  支持参数传递（posts, currentPage, pageSize），这些参数由调用方提供
-->
<body th:fragment="body(posts, currentPage, pageSize)" class="pagination-container">
  <!-- 组件 HTML 内容 -->
  <div class="pagination">
    <a
      th:each="page : ${pages}"
      th:href="@{/archives(page=${page})}"
...
src/components/pagination/
  ...
  └── template.html ← Thymeleaf 模板：定义两个片段（head 和 body）
```
...
<th:block th:insert="~{components/pagination/template :: head}"></th:block>
<!-- ↑ 这样做时：
     1. Vite 会识别组件中的脚本，分析它的 import 语句
     2. 如果脚本中有 import "../../main.css"，Vite 会自动提取 CSS 并创建对应的 <link> 标签
     3. 以上这些都在构建时自动完成，无需手工干预
     4. Thymeleaf 会在这里插入编译后的标签，自动引入组件的资源
-->

<!-- 在 <body> 中使用组件 -->
<th:block th:insert="~{components/pagination/template :: body(posts=${posts}, currentPage=${currentPage})}"></th:block>
...
页面模板中通过 `<th:block th:insert>` 方式引入组件，先在 `<head>` 中声明依赖，再在 `<body>` 中使用：

```html
<!-- 这个示例已在步骤 5a 中展示过，这里说明其工作原理 -->
```

在前面步骤 6 的 `archives.html` 示例中，我们看到使用组件的完整流程：

```html
<head th:remove="tag">
  <!-- 1. 页面的脚本入口（告诉 Vite 为此页面创建独立包） -->
  <script src="/src/scripts/pages/archives.ts" type="module"></script>

  <!-- 2. 引入组件的 head 片段（导入脚本和样式） -->
  <th:block th:insert="~{components/pagination/template :: head}"></th:block>
  <th:block th:insert="~{components/post-list/template :: head}"></th:block>
</head>

<body th:remove="tag">
  <!-- 3. 在 body 中使用组件的 HTML 结构 -->
  <th:block th:insert="~{components/post-list/template :: body(posts=${posts})}"></th:block>
  <th:block th:insert="~{components/pagination/template :: body(currentPage=${currentPage})}"></th:block>
</body>
```
...
1. **在 `<head>` 中插入组件的 head 片段**
   - 语法：`<th:block th:insert="~{components/xxx/template :: head}"></th:block>`
   - 效果：Vite 会识别 `src="/src/components/xxx/main.ts"`，并自动提取关联的 CSS
   - 结果：页面 HTML 中会自动生成 `<script>` 和 `<link>` 标签

2. **在 `<body>` 中插入组件的 body 片段**
   - 语法：`<th:block th:insert="~{components/xxx/template :: body(param1=${var1}, param2=${var2})}"></th:block>`
   - 效果：插入组件的 HTML 结构，并可以传递参数
   - 参数会被传递给模板中的 `th:fragment="body(param1, param2)"`
...
```html
<!-- 具名传参 -->
<th:block th:insert="~{components/xxx/template :: body(param1=${var1}, param2=${var2})}"></th:block>

<!-- 位置传参 -->
<th:block th:insert="~{components/xxx/template :: body(${var1}, ${var2})}"></th:block>
```
